...
def api_get(base_url: str, token: str, path: str, query: Dict[str, object]) -> Dict[str, object]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "weekly-draft-bot/1.0",
...
def resolve_window(week_mode: str, start_date: str, end_date: str) -> WeeklyWindow:
    if start_date and end_date:
        ...
        return WeeklyWindow(start=start, end=end, label=f"{start_date} ~ {end_date}")
...
def pick_opening_options(grouped: Dict[str, List[Dict[str, str]]], rows: List[Dict[str, object]], window: WeeklyWindow) -> List[str]:
    ordered = [
        article
        for section in SECTION_ORDER
        for article in grouped.get(section, [])
    ]

    top_a = short_title(ordered[0]["title"]) if len(ordered) > 0 else "本周技术动态"
    top_b = short_title(ordered[1]["title"]) if len(ordered) > 1 else "工程实践与效率"

    tech_pick = None
    for section in ("技术博客", "开源项目"):
        if grouped.get(section):
            tech_pick = short_title(grouped[section][0]["title"])
            break
    if not tech_pick:
        tech_pick = top_a

    light_pick = None
    for section in ("资源推荐", "网站推荐", "佳文共赏"):
        if grouped.get(section):
            light_pick = short_title(grouped[section][0]["title"])
            break
    if not light_pick:
        light_pick = top_b
    ...
    options = [
        (
            f"这周最大的感受是技术迭代速度继续拉满，{top_a}、{top_b} 这种事件几乎在同一时间线并行发生。"
            "一边是模型与产品能力继续卷上限，一边是工程落地和成本效率进入实战区间。"
            "这期周刊我会沿着‘趋势变化 -> 关键信号 -> 可执行动作’来梳理，尽量把噪音过滤掉。"
        ),
        (
            f"本周我更关注‘怎么把能力落到工程里’这个问题，尤其是 {tech_pick} 这类内容。"
            "相比看热闹，我更在意哪些方法可以被复用、哪些流程可以被自动化、哪些决策会影响长期产能。"
            "所以这期会把重点放在可迁移的思路和可直接试验的实践点上。"
        ),
        (
            f"除了硬核技术，这周也有一些很有意思的观察，比如 {light_pick}。"
            "很多时候真正有价值的启发，不一定来自最热新闻，而是来自那些能改变工作方式的小切口。"
            "这期我会按‘值得知道、值得试试、值得收藏’三个层次来整理。"
        ),
    ]

    # Keep deterministic order but trim excessive spaces.
...
def render_markdown(
    ...
    lines.append("")
    lines.append("## 开篇备选（AI 草稿，择一保留）")
    lines.append("_说明：以下 3 段为草稿备选，建议选 1 段并按你的语气改写。_")
    lines.append("")

    for idx, option in enumerate(opening_options, start=1):
        lines.append(f"### 备选 {idx}")
        lines.append(option)
        lines.append("")
...
def main() -> int:
    ...
    origin = urllib.parse.urlparse(base_url).scheme + "://" + urllib.parse.urlparse(base_url).netloc
    ...
    opening_options = pick_opening_options(grouped, rows, window)
